...
func (device *Device) vitoFsm() (err error) { //, peer *io.ReadWriter, inChan <-chan byte, outChan chan<- byte) {
	...
	for {
		...
		case idle:
			...
			if hasCmd {
				if time.Since(lastEnq) > (1500 * time.Millisecond) {
		...
		case waitAck:
			...
			lastSyn = time.Now()
		case wait:
			if time.Since(lastSyn) > (10 * time.Second) { // TODO: check if 10s timout is ok
				// Emit a sync packet
				state = swP300
				break
			}
		...
		case sendP300Ack:
			...
			if b[0] == ACK {
				...
				err = fmt.Errorf("rid not receive ACK/NACK, going back to wait state")
				log.Debug(err.Error())
